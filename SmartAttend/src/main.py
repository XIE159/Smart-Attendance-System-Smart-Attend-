...
def mark_attendance(class_name):
    ...
    detection_model = load_caffe_model(r'C:\Users\Student\SmartAttend\detection_model\architecture.prototxt', 
                             r'C:\Users\Student\SmartAttend\detection_model\weights.caffemodel')
                ...
                if crop.size > 0:  # Check if crop is not empty
                    ...
                    while True:
                        ...
                        if height >= 1000 or width >= 1000:
                                output_frame = cv2.resize(output_frame, ((width // 2), (height // 2)))
                        else:
                            break      
                    X1, Y1, X2, Y2 = int(detection['x1'] * width), int(detection['y1'] * height), int(detection['x2'] * width), int(detection['y2'] * height)
